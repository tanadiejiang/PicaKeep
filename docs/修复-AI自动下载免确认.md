> 2026-09-09 用户确认：自动下载关闭时仍提供下载工具，但必须确认后入队；仅“下载漫画”总开关控制工具可见性。下文早期“双开关控制可见性”记录已被本次修正取代。设置文案与系统提示同步修正，确认时重新检查总开关。
...
- 执行过程中关闭任一开关：后续调用不自动执行，保留原来的待确认流程。工具可见性只由「下载漫画」总开关决定，子开关关闭不影响工具暴露；页面手动下载行为不变。
